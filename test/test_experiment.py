...
class TestExperimentPropertyTest(TestBase):
    ...
    def test_run_and_analyze_tests(self, logger):
        ...
        tests = [PropertyTest.reliability_leuven, PropertyTest.reliability, PropertyTest.uniqueness_leuven,
                 PropertyTest.uniqueness]
        N = 255
        array_parameter = {
            'n': 16,
            'k': 1,
        ...
        for test_function in tests:
            ...
            exp_rel_leu = create_experiment(N, test_function,
                                            ExperimentPropertyTest.create_noisy_ltf_arrays, array_parameter)
            exp_rel_leu.execute(logger.queue, logger.logger_name)
            self.assertNotEqual(logger.read_result_log(), '')
